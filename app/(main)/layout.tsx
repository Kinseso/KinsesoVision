...
export default function MainLayout({
  ...
  return (
    <div className="flex min-h-screen bg-[#020617]">
      {/* LEFT SIDEBAR - Only visible in the (main) group */}
      <nav className="w-72 bg-zinc-950/90 backdrop-blur-2xl p-6 border-r border-white/5 hidden md:flex flex-col fixed h-full z-50">
        ...
        <div className="flex-1 space-y-1 overflow-y-auto pr-2 custom-scrollbar">
          ...
          {/* All these links now point to pages inside (sectors) */}
          ...
          <Link href="/vault" className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-white/5 font-bold text-sm transition-all group">
            <Icons.Cloud size={18} className="group-hover:text-blue-300" /> Evidence Vault
          </Link>
        </div>
        ...
        <div className="mt-auto pt-6 border-t border-zinc-900">
          <div className="flex items-center gap-3 p-3 bg-zinc-900/40 rounded-2xl border border-white/5">
            ...
            <div className="flex flex-col leading-none">
              <span className="text-[10px] font-black text-white uppercase italic">Founder: Agbes</span>
              <span className="text-[8px] font-bold text-emerald-500 uppercase mt-1">Status: Online</span>
            </div>
          </div>
        </div>
      </nav>
